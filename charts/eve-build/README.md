...
| eveBuild.trackedInfo.dispatchSchedule | string | `"*/30 * * * *"` | Only rendered when rabbitmq.enabled is true - the dispatch CronJob and refresh-worker Deployment below keep every tracked character's/corporation's assets, blueprints, industry jobs and PI colonies cached ahead of time, so a page load hits Redis instead of blocking on ESI. With rabbitmq disabled, that data still refreshes lazily/inline on demand as before - this is a pure performance optimization, not a required feature.  Cron schedule the dispatch CronJob uses to enqueue a refresh job for every tracked character/corporation - comfortably more often than the shortest cache TTL (15 minutes, for PI colonies) so the cache never goes fully cold between runs. |
| eveBuild.trackedInfo.keda | object | `{"cooldownPeriod":120,"pollingInterval":15}` | Shared KEDA settings for the refresh-worker ScaledObject below. Only used when `keda.enabled` is true - see the top-level `keda` section. |
| eveBuild.trackedInfo.keda.cooldownPeriod | int | `120` | How long (in seconds) to wait after the last high metric value before scaling back down. |
| eveBuild.trackedInfo.keda.pollingInterval | int | `15` | How often (in seconds) KEDA polls RabbitMQ for the current queue length. |
| eveBuild.trackedInfo.refreshWorker.keda.maxReplicas | int | `2` | Maximum refresh-worker replicas KEDA will scale up to. |
| eveBuild.trackedInfo.refreshWorker.keda.minReplicas | int | `0` | Minimum refresh-worker replicas KEDA will scale down to - 0 lets it scale to zero between dispatch runs once the refresh_jobs queue drains. |
| eveBuild.trackedInfo.refreshWorker.keda.queueLength | int | `5` | Target number of queued refresh jobs per replica - KEDA scales so that (queue length / queueLength) replicas are running, up to maxReplicas. |
| eveBuild.trackedInfo.refreshWorker.replicas | int | `1` | Number of refresh-worker replicas. Ignored (and no `replicas` field rendered on the Deployment) when `keda.enabled` is true - the ScaledObject's HPA owns replica count instead. Fetches from ESI (only for entries whose cache has actually expired) and writes straight to MongoDB/Redis. |
| eveBuild.trackedInfo.refreshWorker.resources | object | `{"limits":{"cpu":"250m","memory":"256Mi"},"requests":{"cpu":"50m","memory":"64Mi"}}` | Resource requests and limits for the refresh-worker container. |
...
| mongodb.additionalUsers | list | `[]` | Extra readWrite users to create on this same MongoDBCommunity replica set, each scoped to its own database - e.g. so another release (a PR preview environment) can share this replica set instead of running its own. Ignored when `enabled` is false. Each item's connection string is written to its own Secret, "<release-name>-mongodb-<name>-connection" (same shape as the primary user's "<release-name>-mongodb-connection") - point another release's `mongodb.existingSecret` at it, via an ExternalSecret if that release is in a different namespace. |
...
| mongodb.pvcRetentionPolicy | object | `{"whenDeleted":"Retain","whenScaled":"Retain"}` | Whether each member's PersistentVolumeClaim is kept or deleted when the StatefulSet is deleted or scaled down (persistentVolumeClaimRetentionPolicy). `Retain` keeps the PVC around (e.g. to avoid losing data on an accidental scale-down); `Delete` removes it. Requires Kubernetes 1.27+. |
...
| rabbitmq.deploy | bool | `false` | Deploy a RabbitmqCluster custom resource (named "<release-name>-rabbitmq") for this release. Requires the RabbitMQ Cluster Operator (https://github.com/rabbitmq/cluster-operator) already installed in the cluster - this chart only creates the CR, not the operator itself. Ignored when `enabled` is false. Leave false to instead point `url`/`existingSecret` at an external/managed RabbitMQ instance. |
| rabbitmq.enabled | bool | `false` | Enable RabbitMQ connectivity for the app (sets RABBITMQ_ENABLED and wires up RABBITMQ_URL below). Leave disabled if the app has no messaging use yet. Point `url` (or `existingSecret`) at wherever RabbitMQ actually runs - either an external/managed instance, or the RabbitmqCluster this chart can provision itself when `deploy` is also true. |
...
| rabbitmq.replicas | int | `1` | RabbitMQ replica count for the self-deployed cluster. |
| rabbitmq.resources | object | `{"limits":{"cpu":"1","memory":"1Gi"},"requests":{"cpu":"100m","memory":"512Mi"}}` | Resource requests and limits for the self-deployed cluster's rabbitmq container. |
| rabbitmq.storage | string | `"5Gi"` | Storage requested per node's PersistentVolumeClaim, for the self-deployed cluster. |
| rabbitmq.storageClassName | string | `"do-block-storage-xfs"` | StorageClass for the self-deployed cluster's PersistentVolumeClaims. |
| rabbitmq.url | string | `"amqp://guest:guest@localhost:5672/"` | AMQP connection URL for the app to use when `existingSecret` isn't set - e.g. an external/managed RabbitMQ instance, or the self-deployed cluster's Service (<release-name>-rabbitmq.<namespace>.svc.cluster.local). |
